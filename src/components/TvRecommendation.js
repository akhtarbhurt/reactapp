...
export default function TvRecommendation({ id, ScrollToTop, }) {
    const [similar, setSimilar] = useState([]);

    const fetchSimilarData = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=${process.env.REACT_APP_API_KEY}`);
                                ...
                                {similar.map((elem) => (
                                    <div key={elem.id}>
                                        <SwiperSlide className='text-center'>
                                            <Link to={`/TvDetails/${elem.id}`} reloadDocument >
                                            ...
                                            <h2 className='text-white fs-5 mt-3 '>{elem.name}</h2>
                                        </SwiperSlide>
                                    </div>
                                ))}
                            </Swiper>

...
export default function TvSlider({ id }) {
    ...
    const getDataFromApi = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/trending/tv/${timeFrame}?api_key=${process.env.REACT_APP_API_KEY}`);
    ...
    return (
        <>
            <Wrapper>
                <Slide slide={slide} handleClick={handleClick} setTimeFrame={setTimeFrame} title={"Popular Tv Show"} />

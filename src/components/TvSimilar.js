...
export default function TvSimilar({ id, ScrollToTop, handleLoading }) {
    const [similar, setSimilar] = useState([]);
    const fetchSimilarData = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/tv/${id}/similar?api_key=${process.env.REACT_APP_API_KEY}`);
